...
export default {
  ...
  created() {
    ...
    bus.$on("setOrderToken", (token, order_id) => {
      console.log("ACCOMMODATION token detected ", token)
      if (typeof token === 'undefined' || token == null || token.length == 0) {
        console.log('Accommodation no active booking')
      }
      that.token = token
  ...
  methods: {
    ...
    async loadBooking() {
      const that = this;
      this.debug>4 && console.log("loadBooking() accommodation booking data token=", this.token, `url = ${this.tour.id}/${this.order_id}/${this.token}`)
      await axios.get(
